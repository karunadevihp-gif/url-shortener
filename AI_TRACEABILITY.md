...
| **Distributed Scaling** | *"Apply the limit across multiple app instances behind a load balancer."* | Initial design used per-JVM memory buckets. | **Refactored** | Replaced with Redis-backed Bucket4j via `LettuceBasedProxyManager` so the 10 req/min/IP limit is shared across containers. |
| **Persistence Layer**[cite: 1] | *"Create JPA database mapping."*[cite: 1] | JPA Entity with hibernate auto-ddl (`update`). | **Modified** | Replaced `hibernate.ddl-auto=update` with a Docker-ready Postgres bootstrap plus migration-capable Liquibase setup for schema control and safe startup. |
| **Distributed Rate Limiting**[cite: 1] | *"Scale the rate limiter across multiple app instances."* | Initial in-memory `ConcurrentHashMap` keyed by IP. | **Refactored** | Replaced with Redis-backed Bucket4j using `LettuceBasedProxyManager` so requests are counted in shared Redis state rather than a single JVM instance. |
...
| **Deployment Readiness**[cite: 1] | *"Provide a multi-container local environment."*[cite: 1] | Docker Compose skeleton with database and cache services. | **Accepted** | Added PostgreSQL, Redis, health checks, and app orchestration with application-level wiring for evaluation and local validation[cite: 1]. |

---

## 3. Human Engineer Sign-Off Log

- [x] **Security Verification**: Confirmed URL input validation, open-redirect protection, and Redis-backed rate limiting on creation endpoints.
...
- [x] **Production Readiness**: Validated PostgreSQL/Redis startup ordering and multi-container `docker-compose.yml` infrastructure setup for a distributed prototype.
...
1. **Distributed Rate Limiting**: The rate limiter is now backed by Redis, which removes the per-instance enforcement problem for multi-node deployments. Operationally, it still assumes a single Redis instance or a managed Redis cluster; a production deployment should tune Redis persistence and availability.
...
3. **Operational Resilience**: The Docker prototype runs with PostgreSQL and Redis; a production deployment should add backups, failover, and environment separation.
